...
public class MainWrapper {
  ...
  public static void main(String[] args) {
    ...
    String user_name = sc.nextLine(); // 공백이 nextline?
    sc.close();
    ...
    try {
      ...
      sql += "VALUES(USER_SEQ.NEXTVAL, ?, ?, SYSDATE)"; // 변수가 들어가는 자리는 ?가 들어간다.
    
      // 쿼리문 작성할때 세미콜론 ; 작성하지마라. (구문오류)
      // PreparedStatement  객체 생성
      ps = con.prepareStatement(sql);
      
      // 쿼리문에 변수 넣기 (문자열 채우기)
      ps.setString(1, user.getUser_id());     // 1번째 물음표 ← user.getUser_id()
      ps.setString(2, user.getUser_name());   // 2번째 물음표 ← user.getName_id()
