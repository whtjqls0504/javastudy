...
public class Library {
  ...
  private void addBook() throws InputMismatchException {
    ...
    int price = sc.nextInt(); // 정수 입력을 안했을 때 InputMismatchException
    // books 리스트에 추가 
  ...
  private void modifyBook() throws RuntimeException, InputMismatchException {
    ...
    System.out.println("수정할 책의 제목을 입력하시오 >>> ");
    String title = sc.next();
    if(title.isEmpty()) {
    ...
    for(Book book : books) {
      if(title.equals(book.getTitle())) {
        System.out.println("수정할 책의 새 제목 입력 >>> ");
        String newtitle = sc.next();
        book.setTitle(title);
        System.out.println(title  + " 책의 이름이 " + newtitle + "으로 수정되었습니다.");
        
        System.out.println("수정할 책의 가격 입력 >>> ");
        int price = sc.nextInt();
        book.setPrice(price);
        System.out.println(newtitle  + " 책의 가격이 " + price + "원으로 수정되었습니다.");
        return;
      }
    }
    System.out.println(title + " 제목을 가진 책이 없습니다.");
   ...
   */
  public void manage() {
    // 사용자가 종료 명령을 내릴때 까지 프로그램은 종료되지 않는다.
