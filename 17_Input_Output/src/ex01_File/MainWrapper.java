...
public class MainWrapper {
   ...
   * 2. 리눅스 : 슬래시(/)
   * 3. java.io.File 클래스에서는 플랫폼에 따라서 경로 구분자를 자동으로 바꿔주는 separator 필드 값이 있다.
   */
  ...
  public static void ex02() { 
    ...
    try {
      ...
      // 파일이 있으면 지우고, 없으면 만들기
      if(file.exists()) {
        file.delete();    // 파일이 존재하면 파일을 즉시 삭제하라.
        System.out.println("myfile.txt 파일 삭제 ");
      } else {
        file.createNewFile();   // 반드시 예외 처리를 해야하는 코드 (Checked Exception인 IOException 발생)새로 만들어라.
        System.out.println("myfile.txt 파일 생성 완료");
      
      }
    
    } catch(IOException e) {
      e.printStackTrace();  // 실행- > 예외 발생했을때 예외 발생 위치 오류 생성
    }
  } 
  ...
  public static void ex03() {
    ...
    // 디렉토리에 잇는 모든 File 객체 가져오기
    File[] files = dir.listFiles();   // 디렉토리 배열로 가져오기
    ...
    for(int i=0; i<files.length; i++) {
      ...
      // File 객체 최종수정일
      long lastModified = file.lastModified(); // long 타입의 날짜는 타임스탬프
