...
public class MainWrapper {
  ...
  // 디렉터리를 File 객체로 만들기
    ...
    try {
      ...
      // read(byte[] b) 메소드
      // 1. 파라미터로 전달된 byte[] b에 읽은 내용을 저장한ㄷ.ㅏ
      // 2. 실제로 읽은 바이트 수를 반환한다.
      // 3. 읽은 내용이 없으면 -1을 반환한다.
      
      readByte = fin.read(b); // b배열에 저장된 숫자 4
      sb.append(new String(b, 0, readByte));    // 실제로는 4번
   ...
   * 1. 내부 버퍼를 가지고 있는 입력스트림 
   * 2. 많은 데이터를 한 번에 입력 받기 때문에 속도 향상을 위해서 사용한다.
